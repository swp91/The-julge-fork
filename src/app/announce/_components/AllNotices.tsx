...
const AllNotices = () => {
  ...
  const sortedNotices = [...filteredNotices].sort((a, b) => {
    const isExpiredA = new Date(a.startsAt) < new Date();
    const isExpiredB = new Date(b.startsAt) < new Date();
    const isClosedA = a.closed;
    const isClosedB = b.closed;

    if (isClosedA || isExpiredA) return 1;
    if (isClosedB || isExpiredB) return -1;
    return 0;
  });

  const postsPerPage = 6;
  const totalPages = Math.ceil(sortedNotices.length / postsPerPage);
  const startIndex = (currentPage - 1) * postsPerPage;
  const currentPosts = sortedNotices.slice(
    startIndex,
